...
parser.add_argument(
    '-c',
    '--checkpoint',
    default='/scratch/xiaolonw/pytorch_checkpoints/unsup3dnl_single_contrast',
    type=str,
    metavar='PATH',
...
parser.add_argument('--topk_vis', default=20, type=int, help='topk_vis')

parser.add_argument('--videoLen',
                    default=8,
                    type=int,
...
parser.add_argument('--cropSize',
                    ...
                    type=int,
                    help='predict how many frames away')
parser.add_argument('--cropSize2',
                    default=80,
                    type=int,
                    help='predict how many frames away')
parser.add_argument('--temporal_out',
                    default=4,
                    type=int,
                    help='predict how many frames away')
...
def main():
    ...
    model = video3d.CycleTime(class_num=params['classNum'],
                              trans_param_num=3,
...
def test(val_loader, model, epoch, use_cuda):
    ...
    for batch_idx, (imgs_total, patch2_total, lbls,
        ...
        for i in range(lbls.shape[0]):
            ...
            for j in range(nowlbl.shape[0]):
                for k in range(nowlbl.shape[1]):
                    ...
                    for t in range(len(lbl_set)):
                        if lbl_set[t][0] == pixellbl[0] and lbl_set[t][
                                1] == pixellbl[1] and lbl_set[t][2] == pixellbl[
                                    2]:
        ...
        for iter in range(total_frame_num - finput_num_ori):
            ...
            corrfeat2 = corrfeat2.view(corrfeat2.size(0), height_dim, width_dim,
                                       height_dim, width_dim)
